  ...
  methods: {
    ...
    */
    async assignRole() {
      ...
      this.loading = true
      const result = await this.$http.user.assignRoles(userId, roleIds)
      this.loading = false
    ...
    */
    revomveUsers() {
      this.$confirm('确定删除吗?').then(async () => {
        await this.$http.user.removeUsers(this.selectedIds)
    ...
    */
    async removeUser(id) {
      await this.$http.user.removeById(id)
      this.$message.success('删除成功')
    ...
    addOrUpdate() {
      this.$refs.userForm.validate(valid => {
        if (valid) {
          const { user } = this
          this.loading = true
          this.$http.user[user.id ? 'update' : 'add'](user).then((result) => {
            this.loading = false
